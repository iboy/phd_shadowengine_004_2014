		...
		public void OnOSCMessageReceived(object sender,UniOSCEventArgs args){
			...
			if(IsLearning){
				...
				foreach(var d in oscSessionItemList){
					if(d.isLearning){d.address = args.Message.Address;}
				}
				...
				if(_osi == null) return;
				if(args.Message.Data == null)return;
				int dc = args.Message.Data.Count;
				for(int i = 0;i< dc;i++){
					...
					_osi.data[i] = args.Message.Data[i].ToString();
					//only once we get the type .( performance concerns)
					if(String.IsNullOrEmpty(_osi.dataTypeList[i])) _osi.dataTypeList[i] = args.Message.Data[i].GetType().FullName;
				}//for
