...
namespace UniOSC{
	...
	public class UniOSCEventTargetCBImplementation : UniOSCEventTargetCB{
		...
		{
			Debug.Log("UniOSCEventTargetCBImplementation.OnOSCMessageReceived:"+args.Message.Address);

			//args.Address
			//args.Message
			//args.Message.Data  (get the data as an object[] array)
			...
			* a) 
			* if(( args.Message.Data[0] is System.Single)
			* 
			* b) 
			* if( args.Message.Data[0].GetType() == typeof(System.Single))
			* 
			...
			* if(args.Message.TypeTag.Substring(1,1) == "f")
			*/
